...
select art.title as TitleOfArtworks from Artworks art join ExhibitionArtworks ea1 on art.ArtworkID=ea1.ArtworkID
join Exhibitions e1 on e1.ExhibitionID=ea1.ExhibitionID join ExhibitionArtworks ea2 on art.ArtworkID=ea2.ArtworkID
join Exhibitions e2 on e2.ExhibitionID=ea2.ExhibitionID where e1.Title = 'Modern Art Masterpieces' 
...
select e.Title from Exhibitions e join ExhibitionArtworks ea on e.ExhibitionID=ea.ExhibitionID join Artworks art on art.ArtworkID=ea.ArtworkID
join Artists a on a.ArtistID = art.ArtistID where a.Name in ('Leonardo da Vinci','Vincent van Gogh') group by e.Title having Count(distinct a.ArtistID)=2
...
where c.CategoryID = (select distinct c.CategoryID  from Artworks art 
join Categories c ON art.CategoryID = c.CategoryID  where art.Title = 'Mona Lisa' )
